...
// 01 준비: 금액 파이프라인
function feFigPre(){
  var items=[['추정가격','부가세 제외','계약방법·공고기간 결정'],['기초금액','설계금액 기반','복수예비가격 ±2%'],['예정가격','비공개','낙찰자 결정 기준'],['낙찰금액','낙찰률=낙찰÷예정','70% 미만은 저가낙찰'],['계약금액','낙찰금액+부가세','보증금·지체상금의 기준']];
...
// 02 체결: 보증금 비율 막대
function feFigSign(){
  var rows=[['입찰보증금','입찰금액',5,FE_C.surf],['계약보증금 · 물품·용역','계약금액',10,FE_C.mp],['계약보증금 · 공사','계약금액',15,FE_C.gs],['공사이행보증서','계약금액',40,FE_C.gs],['저가낙찰 이행보증 (예정가 70% 미만)','계약금액',50,FE_C.red]];
...
function feFigAfter(){
  var s='', W=880, y=56;
  var st=[['이의신청','행위일 20일 / 안 날 15일'],['중앙관서 심사','15일 이내 통지'],['재심 청구','통지 후 20일'],['분쟁조정위 조정','50일 이내 · 재판상 화해 효력'],['소송','행정·민사']];
  var bw=150, gap=30;
...
var FE_EV = {
  ...
  haeje:{ n:'계약 해제', docs:['계약 해제 통보서','계약보증금 귀속 통보서'],
    rows:[['조건','이행 착수 전 또는 이행 불능'],['공사','계약보증금 15% 국고 귀속'],['물품','계약보증금 10% 국고 귀속'],['용역','계약보증금 10% 국고 귀속'],['제재','손해배상·부정당업자 제재 가능']]},
...
const STEP_EXTRA = {
  ...
  sign:{check:['계약서·계약보증금 납부 확인','청렴계약 이행 서약','선금 신청 여부 확인'],
        trap:['해제 시 계약보증금 귀속: 공사 15% / 물품·용역 10%','구두 약정은 효력 없음 — 서면 원칙']},
    ...
    items:[
      { name:'일반 물품 (제조·구매)', tag:'기본형',
        rows:[
          ...
          ['낙찰자 결정','최저가 또는 적격심사 (물품 낙찰하한율 84.245%·고시금액 미만)'],
          ['발주문서','규격서 · 구매사양서 · 시방서'],
          ['보증금','입찰 5% / 계약 10%'],
